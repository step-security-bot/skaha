...
def test_get_event_loop_existing():
    ...
    # Assertions
    assert retrieved_loop is loop  # The retrieved loop should be the same as the created loop
...
def test_get_event_loop_new():
    ...
    with patch('asyncio.get_event_loop', side_effect=RuntimeError):
        ...
        # Assertions
        assert isinstance(retrieved_loop, asyncio.AbstractEventLoop)  # Should return a new event loop
        assert retrieved_loop.is_running() is False  # The new loop should not be running
